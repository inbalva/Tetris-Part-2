...
char TetrisGame::runGame()
{
	...
	while (true)
	{
		...
		if (_kbhit()) // checks if there is anything in the buffer
		{
			...
			else if (keyPressed == Point::direction::KEY_UP && (type != blockType::JOKER && type != blockType::BOMB))
			{
		...
		if (type != blockType::JOKER || (type == blockType::JOKER && (keyPressed != TetrisGame::menu::STOP_JOKER1 && keyPressed != TetrisGame::menu::STOP_JOKER2)))
		{
...
void TetrisGame::checkBucket(bool &createNewBlock)
{
	...
	if (stopped == true)
	{
		...
		if (type == blockType::JOKER && numOfDeletedRows == 1)
		{
			updateScore(50);
			printBucket();
		}
